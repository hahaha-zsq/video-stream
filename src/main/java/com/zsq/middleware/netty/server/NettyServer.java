...
public class NettyServer {
    ...
    private void initializeServer() throws InterruptedException {
        // 创建InetSocketAddress，用于绑定端口
        InetSocketAddress socketAddress = new InetSocketAddress("0.0.0.0", properties.getServer().getPort());
        // 配置线程组大小
        ...
        bootstrap.group(bossGroup, workerGroup)
                ...
                //选择直接内存
                .option(ChannelOption.ALLOCATOR, PreferredDirectByteBufAllocator.DEFAULT) // 选择直接内存
                ...
                .childOption(ChannelOption.SO_RCVBUF, 128 * 1024) // 设置接收缓冲区大小，提升接收效率，防止数据丢失，32KB大小是比较常见的调整。
                .childOption(ChannelOption.SO_SNDBUF, 1024 * 1024)// 设置发送缓冲区大小，提升发送效率。
                .childOption(ChannelOption.WRITE_BUFFER_WATER_MARK, new WriteBufferWaterMark(1024 * 1024 / 2, 1024 * 1024)); //设置发送缓冲区高水位线和低水位线，当发送缓冲区超过高水位线时，会触发高水位事件，当发送缓冲区低于低水位线时，会触发低水位事件。
